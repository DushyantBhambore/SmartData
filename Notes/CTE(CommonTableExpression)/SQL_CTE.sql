...
WITH EmployeeToUpdate AS (
    SELECT EmployeeID ,'Vaswani' AS NewLastName
    FROM Employeesss
...
-- Update the data in the Employeesss table based on the CTE
UPDATE Employeesss
SET LastName = (SELECT NewLastName FROM EmployeeToUpdate WHERE Employeesss.EmployeeID = EmployeeToUpdate.EmployeeID)
WHERE EmployeeID IN (SELECT EmployeeID FROM EmployeeToUpdate);
